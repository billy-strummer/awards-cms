...
const ticketModule = {
    ...
    try {
      ...
      const cards =
        (types || [])
          .map((t) => {
            const eb =
              t.early_bird_price && new Date(t.early_bird_deadline) > new Date()
      ...
      el.innerHTML =
        `<div class="d-flex justify-content-between mb-3"><h5 class="mb-0">Ticket Types</h5>
  ...
  async saveTicketType(eventId) {
    ...
    if (!payload.name || isNaN(payload.price) || isNaN(payload.quantity)) {
      utils.showToast('Name, price and quantity are required.', 'warning');
      return;
    }
    try {
  ...
  async initTicketPurchase(eventId) {
    ...
    try {
      ...
      const rows = (types || [])
        .map((t) => {
          const price =
            t.early_bird_price && new Date(t.early_bird_deadline) > new Date() ? t.early_bird_price : t.price;
      ...
      el.querySelectorAll('.ticket-qty').forEach((inp) =>
        inp.addEventListener('input', () => {
          let total = 0;
          el.querySelectorAll('.ticket-qty').forEach((i) => {
            total += (parseFloat(i.dataset.price) || 0) * (parseInt(i.value, 10) || 0);
          });
          document.getElementById('ticketTotal').textContent = `\u00a3${total.toFixed(2)}`;
        })
      );
    } catch (err) {
      utils.showToast('Failed to load purchase UI: ' + err.message, 'error');
    } finally {
      utils.hideLoading();
    }
  },
  ...
  async createTicketCheckout(eventId) {
    ...
    if (!tickets.length) {
      utils.showToast('Select at least one ticket.', 'warning');
      return;
    }
    try {
      utils.showLoading();
      ...
      utils.showToast('Checkout error: ' + err.message, 'error');
    } finally {
      utils.hideLoading();
    }
  },
  ...
  async generateETicket(ticketId) {
    try {
      utils.showLoading();
      const result = await apiClient.select('event_guests', {
        select: '*, events(event_name, event_date, venue)',
        filters: { id: { eq: ticketId } },
        pageSize: 1,
      });
      ...
      doc.text('E-TICKET', 74, 22, { align: 'center' });
      ...
      doc.setFont('helvetica', 'bold');
      doc.text('GUEST', 10, 71);
      doc.setFont('helvetica', 'normal');
      doc.text(guest.guest_name || '-', 10, 79);
      ...
      if (guest.dietary_requirements) {
        ...
        doc.text(`Dietary: ${guest.dietary_requirements}`, 10, 100);
      }
      if (window.QRCode) {
        const canvas = document.createElement('canvas');
        new window.QRCode(canvas, {
          text: `BTA-TICKET:${ticketId}`,
          width: 80,
          height: 80,
      ...
      utils.showToast('Failed to generate e-ticket: ' + err.message, 'error');
    } finally {
      utils.hideLoading();
    }
  },
  ...
  async renderGuestPreferences(guestId) {
    ...
    try {
      utils.showLoading();
      const result = await apiClient.select('event_guests', {
        select: 'id, guest_name, dietary_requirements, notes',
        filters: { id: { eq: guestId } },
        pageSize: 1,
      });
      ...
      const dietOptions = [
        ...
        .map((o) => `<option value="${o}" ${(g.dietary_requirements || 'None') === o ? 'selected' : ''}>${o}</option>`)
        .join('');
      ...
      utils.showToast('Failed to load preferences: ' + err.message, 'error');
    } finally {
      utils.hideLoading();
    }
  },
  ...
  async saveGuestPreferences(guestId) {
    ...
    try {
      utils.showLoading();
      await apiClient.update('event_guests', guestId, {
        dietary_requirements: dietary === 'None' ? null : dietary,
        notes,
  ...
  async renderTicketDashboard(eventId) {
    ...
    try {
      ...
      const typeRows =
        (types || [])
          .map(
            (
              t
            ) => `<tr><td>${utils.escapeHtml(t.name)}</td><td>&pound;${parseFloat(t.price).toFixed(2)}</td><td>${t.quantity}</td>
        <td>${t.includes_table ? '<span class="badge bg-success">Yes</span>' : '<span class="badge bg-secondary">No</span>'}</td></tr>`
          )
          .join('') || '<tr><td colspan="4" class="text-center text-muted">No ticket types defined.</td></tr>';
      el.innerHTML = `<div class="row g-3 mb-3">${stats
        .map(
          (s) => `<div class="col-6 col-md-3">
        <div class="card text-center shadow-sm border-${s.color}"><div class="card-body py-3">
          ...
          <div class="text-muted small">${s.label}</div></div></div></div>`
        )
        .join('')}</div>
      ...
      utils.showToast('Dashboard load failed: ' + err.message, 'error');
    } finally {
      utils.hideLoading();
    }
  },
  ...
  async addToWaitlist(eventId, email, name) {
    ...
    if (!utils.isValidEmail(email)) {
      utils.showToast('Invalid email address.', 'warning');
      return false;
    }
    try {
      utils.showLoading();
      await apiClient.insert('event_waitlist', {
        event_id: eventId,
        email,
        name,
        status: 'waiting',
      ...
      utils.showToast('Waitlist error: ' + err.message, 'error');
      return false;
    } finally {
      utils.hideLoading();
    }
    ...
    try {
      ...
      const [types, waitlist, guests] = await Promise.all([
        ...
        apiClient.selectAll('event_waitlist', {
          select: '*',
          filters: { event_id: { eq: eventId }, status: { eq: 'waiting' } },
          sort: { column: 'created_at', ascending: true },
        }),
        apiClient.selectAll('event_guests', {
          select: 'id',
          filters: { event_id: { eq: eventId }, rsvp_status: { neq: 'cancelled' } },
        }),
      ]);
      const capacity = (types || []).reduce((s, t) => s + (t.quantity || 0), 0);
      const available = Math.max(0, capacity - (guests || []).length);
      if (!available) {
        utils.showToast('No available spots.', 'info');
        return;
      }
      if (!(waitlist || []).length) {
        utils.showToast('Waitlist is empty.', 'info');
        return;
      }
      ...
      utils.showToast('Waitlist processing error: ' + err.message, 'error');
    } finally {
      utils.hideLoading();
    }
  },
  ...
  async processRefund(ticketId) {
    if (
      !(await utils.confirmDialog({
        title: 'Cancel Ticket',
        message: 'Cancel this ticket and issue a refund?',
    ...
    try {
      utils.showLoading();
      const guestResult = await apiClient.select('event_guests', {
      ...
      await apiClient.update('event_guests', ticketId, {
      ...
      utils.showToast('Refund error: ' + err.message, 'error');
    } finally {
      utils.hideLoading();
    }
  },
