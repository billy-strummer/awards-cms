...
- ~~`api/judge-automation.js` occupies a function slot with zero HTTP callers.~~ **Resolved as part of the fix above** — this file is now the real, live HTTP entry point for the daily automation cron, not dead weight.
...
## 11. Automation — Production Cron Implementation

A follow-up "professionally maintained product" pass produced a full operational documentation set (`DEPLOYMENT-GUIDE.md`, `DISASTER-RECOVERY.md`, `OPERATIONS-MANUAL.md`, `MONITORING.md`, `ENVIRONMENT-VARIABLES.md`, `SECURITY-REVIEW-FINAL.md`, `ROADMAP-V2.md`) and, in writing it, surfaced the automation-scheduler gap referenced (as historical technical debt) in Section 7. Rather than just document it, that gap has now been fixed:

- **Architecture**: Vercel Cron (`vercel.json`'s `crons` array) invokes `/api/judge-automation?action=cron-tick` once daily. Vercel automatically authenticates the request with `Authorization: Bearer $CRON_SECRET`; the endpoint verifies it via constant-time comparison and fails closed (500, not a silent pass-through) if `CRON_SECRET` isn't configured.
- **`api/judge-automation.js`** was chosen as the real HTTP entry point specifically because — before this fix — it had zero live callers, so wiring the cron through it uses an existing function slot rather than requiring a 13th (Vercel's Hobby cap remains 12/12).
- **`api/_lib/automation-scheduler.js`** was rewritten: the `node-cron`-based scheduling (which never actually ran on Vercel — see Section 7's historical note) was removed entirely, replaced with `runDailyAutomation()`, which always runs the daily-cadence tasks (deadline reminders, payment reminders, scheduled campaign dispatch, the judging-deadline check) and additionally runs the weekly tasks (judge progress reports, weekly stats) on Monday and GDPR retention cleanup on Sunday — both checked in Europe/London time regardless of the server's own timezone.
- **Reliability**: each sub-task is isolated (`try`/`catch` per task) so one failure doesn't prevent the others from running.
- **Idempotency/retry-safety**: verified that every sub-task's existing domain logic already de-dupes against database state (an "already sent today" check for reminders, an immediate status flip to prevent double-dispatch for campaigns, status-filtered processing for shortlist generation) — confirmed both by the automated test suite and a live manual run.
- **Observability**: every task and the overall run logs to `console.log`/`console.error` (visible in Vercel's function logs), and the endpoint returns a structured JSON summary of every task attempted and its outcome.
- **Verified live**: a manual `curl` invocation against this project's test database during this pass completed successfully in ~4.5 seconds, correctly ran only the day-appropriate subset of tasks, and its console output confirmed full observability end to end — not just a passing unit test in isolation.
- **Known, accepted limitation**: Vercel Hobby's cron constraints (max 2 jobs, daily-minimum interval, UTC-only schedules) mean scheduled email campaigns now dispatch within 24 hours of their scheduled time rather than the ~5 minutes the original (never-working) design intended. Documented as an accepted trade-off in `DEPLOYMENT-GUIDE.md` §11, with the config-only upgrade path (a second, more frequent cron entry on Vercel Pro) noted for if tighter timing is ever needed.

Full detail: `DEPLOYMENT-GUIDE.md` §11, `ENVIRONMENT-VARIABLES.md`'s Automation section, and `RELEASE-CANDIDATE-SIGNOFF.md`.

**This resolves the single most significant risk identified in the prior pass.** Section 7's technical debt list has been updated accordingly.
