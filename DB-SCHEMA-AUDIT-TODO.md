...
### [ ] DB-C1 — `award_years` table referenced in 20+ FKs but not defined in root SQL files
...
### [ ] DB-C2 — `events` table defined twice with incompatible column names
...
### [ ] DB-C3 — RLS (Row Level Security) missing on 8 critical tables
...
### [ ] DB-H1 — Missing indexes on high-query columns
...
### [ ] DB-H2 — `email_campaigns` foreign keys applied via DO block that can silently fail
...
### [ ] DB-H3 — `regions` table missing `created_at` and `updated_at` timestamps
...
### [ ] DB-M1 — `judge_conflicts.org_id` is a soft FK with no referential integrity
...
### [ ] DB-M2 — `cms_audit_logs` missing `updated_at` column
...
### [ ] DB-M3 — Inconsistent FK convention: some hard constraints, some soft/commented
...
### [ ] DB-L1 — Redundant `IF NOT EXISTS` checks around `CREATE OR REPLACE FUNCTION`
