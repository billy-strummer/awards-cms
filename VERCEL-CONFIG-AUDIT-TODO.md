...
### [x] VC-C1 — No SPA catch-all rewrite in `vercel.json`
...
### [x] VC-C2 — Undocumented environment variables used in production API code
...
### [x] VC-H1 — CSP missing `form-action` directive
...
### [x] VC-H2 — `'unsafe-inline'` in `style-src` weakens CSP
...
### [x] VC-H3 — No automated guard against exceeding 12-function Vercel limit
...
### [x] VC-M1 — `public-winners.html` missing from build copy list
...
### [x] VC-M2 — Unusual `frame-src` entries for TradingView widgets
...
### [x] VC-L1 — ESLint failures do not halt the build
