...
- **Imports remain practical**: CSV import batches inserts in groups of 50 with server-side validation before any write — this design should scale to the stated volume without special handling, consistent with the prior pass's assessment. Still not tested at real volume (test database holds dozens of records) — worth a real test batch before your first large import.
...
- [ ] If you ran a real CSV import at meaningful volume, confirm timing was acceptable and no rows silently failed.
