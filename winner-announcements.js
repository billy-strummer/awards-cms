...
window.winnerAnnouncementsModule = {
  ...
  async openAnnouncementWizard() {
    this._wizard = { step:1, selectedWinners:[], channels:[], scheduleAt:null, emailTemplateId:null, socialPlatforms:[], socialTemplate:null };
    this._removeModal('annWizModal');
    const modal = document.createElement('div');
    modal.className = 'modal fade'; modal.id = 'annWizModal'; modal.setAttribute('tabindex','-1');
  ...
  async _stepContent(step) {
    ...
    if (step === 2) {
      ...
      ].map(([v,l,icon,desc]) => `<div class="card p-3 mb-2">
    ...
    if (step === 3) {
      ...
      if (channels.includes('social')) {
        html += `<div class="mb-2"><label class="form-label fw-semibold">Platforms</label>
          <div class="d-flex gap-3">${['twitter','linkedin','facebook','instagram'].map(p=>
            `<div class="form-check"><input class="form-check-input soc-p" type="checkbox" id="sp_${p}" value="${p}" checked>
             <label class="form-check-label text-capitalize" for="sp_${p}">${p}</label></div>`).join('')}</div></div>
  ...
  _stepListeners(step) {
    if (step === 1) {
      document.getElementById('wizSelAll')?.addEventListener('click', () =>
        document.querySelectorAll('.winner-chk').forEach(c => c.checked = true));
    }
    if (step === 4) {
  ...
  async _next() {
    ...
    if (step === 1) {
      ...
      if (!sel.length) { utils.showToast('Select at least one winner.','warning'); return; }
      ...
      if (!this._wizard.channels.length) { utils.showToast('Select at least one channel.','warning'); return; }
    } else if (step === 3) {
  ...
  async _execute() {
    ...
    const results = [];
    try {
      if (channels.includes('website')) { await this.publishToWebsite(ids, scheduleAt); results.push('Website published'); }
      if (channels.includes('email') && emailTemplateId) { await this.sendWinnerEmails(ids, emailTemplateId, scheduleAt); results.push('Emails queued'); }
      if (channels.includes('social')) { await this.createSocialPosts(ids, socialPlatforms, socialTemplate, scheduleAt); results.push('Social drafts created'); }
      utils.showToast(results.join(' | '), 'success');
      bootstrap.Modal.getInstance(document.getElementById('annWizModal'))?.hide();
    } catch(e) { utils.showToast('Execution error: ' + e.message, 'error'); }
  },
  ...
  async renderAnnouncementLog(containerId = 'announcementLogContainer') {
    ...
    try {
      ...
      </tr>`).join('') || '<tr><td colspan="6" class="text-muted text-center py-3">No announcements logged yet</td></tr>';
      el.innerHTML = `<div class="table-responsive"><table class="table table-sm table-hover">
        ...
        <tbody>${rows}</tbody></table></div>`;
    } catch(e) { el.innerHTML = `<div class="alert alert-danger small">${utils.escapeHtml(e.message)}</div>`; }
  },
  ...
  async openEmbargoModal(winnerIds) {
    ...
    const modal = document.createElement('div');
    modal.className = 'modal fade'; modal.id = 'embargoModal'; modal.setAttribute('tabindex','-1');
    ...
    document.getElementById('confirmEmbargo').addEventListener('click', async () => {
      ...
      if (!val) { utils.showToast('Please select a date.','warning'); return; }
  ...
  async generatePressRelease(winnerIds) {
    try {
      ...
      const modal = document.createElement('div');
      modal.className = 'modal fade'; modal.id = 'prModal'; modal.setAttribute('tabindex','-1');
      ...
      document.body.appendChild(modal);
      new bootstrap.Modal(modal).show();
      document.getElementById('prCopy')?.addEventListener('click', () =>
