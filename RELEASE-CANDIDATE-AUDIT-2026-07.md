...
- RLS enabled on 112 of 115 tables; the 3 exceptions (`email_logs`, `judge_conflicts`, `sponsorships`) were checked empirically — **no role (`anon` or `authenticated`) has any grant on them at all**, so the missing RLS isn't currently exploitable. Noted as a defense-in-depth gap, not an active one.
