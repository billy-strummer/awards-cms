...
> **Follow-up found 2026-07 during production-deployment verification, applied via migration 077**: this item was correctly fixed for 6 of the 8 tables, but live verification against a real Supabase project (not a bare local Postgres instance) found two problems the original fix didn't catch:
> 1. `award_assignments` and `event_guests` still carry an *older* `"Allow all access to X" ... USING (true)` policy from `migrations/000-complete-database-setup.sql` that the fix below never dropped. Postgres combines multiple `PERMISSIVE` policies with `OR`, so that leftover policy silently cancels out `no_direct_client_access`'s `USING (false)` — both tables show RLS as "enabled" but are currently just as open to `anon`/`authenticated` as if this fix had never run. **Not yet fixed** — flagged for explicit follow-up, not fixed unilaterally, since it touches two tables beyond what was authorized in that session.
> 2. Three *other* tables outside this item's original 8 — `email_logs`, `judge_conflicts`, `sponsorships` — had the same missing-RLS problem and, on the real Supabase project, also had the standard broad default grants to `anon`/`authenticated`, making them genuinely exploitable (not just theoretical): a `POST` with the public anon key could insert/update/delete rows directly, confirmed and then blocked via migration 077. See `RELEASE-CANDIDATE-AUDIT-2026-07.md` for the full writeup — that report's original claim that these 3 tables were "not currently exploitable" was based on a local Postgres replica that didn't reproduce Supabase's own default grants, and was wrong for the real project.
>
> **Before applying this same fix to `british-trade-awards-cms` (production)**: run the 3-part verification query in `PRODUCTION-DEPLOYMENT-PLAN.md` Step 2, which checks for both problems above, not just whether `pg_tables.rowsecurity = true`.
