-- ============================================
-- USE TENANT BRANDING IN CONFIRMATION EMAIL
-- ============================================
-- The send_entry_confirmation_email function was hardcoded with
-- "British Trade Awards" branding. This migration updates it to
-- read from the tenant_branding table so that white-label tenants
-- get their own company name, colours, logo, and contact details.
-- ============================================
...
BEGIN
  ...
  -- Build header: use logo if configured, otherwise text
  IF v_logo_url != '' THEN
    v_header_html := '<div style="background:linear-gradient(135deg,' || v_primary_color || ' 0%,' || v_secondary_color || ' 100%);padding:32px;text-align:center;border-bottom:3px solid ' || v_accent_color || ';">'
      || '<img src="' || v_logo_url || '" alt="' || v_brand_name || '" style="max-height:60px;display:block;margin:0 auto;">'
      || '<p style="color:' || v_accent_color || ';margin:12px 0 0;font-size:13px;letter-spacing:2px;text-transform:uppercase;opacity:0.9;">Self-Nomination Entry Confirmation</p>'
      || '</div>';
  ELSE
    v_header_html := '<div style="background:linear-gradient(135deg,' || v_primary_color || ' 0%,' || v_secondary_color || ' 100%);padding:32px;text-align:center;border-bottom:3px solid ' || v_accent_color || ';">'
